...
object PnmlOwnParser {
    ...
    private fun Element.attrOpt(name: String) = if (hasAttribute(name)) getAttribute(name) else null
...
//            println("transitions:")
            transitions.forEach { transition ->
                ...
                val promInvisible = transition.elementOpt("toolspecific")
                        ?.attrOpt("activity")?.equals("\$invisible\$") ?: false
                val invisible = name.isNullOrEmpty() || promInvisible

                val pnTransition = petrinet.addTransition(name ?: id)
                // should be ignored in logs
                pnTransition.isInvisible = invisible
