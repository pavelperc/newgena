...
//        graph["label" eq graphLabelStr]
        ...
        petrinet.transitions.forEach { tr ->
            var labelStr = ""
            if (drawTransitionNames) labelStr += tr.label
            if (drawTransitionNames && drawTransitionIds) labelStr += "\n"
            if (drawTransitionIds) labelStr += tr.pnmlId
            mutNode(tr.pnmlId).add(
                    Shape.RECTANGLE,
                    if (tr.isInvisible) Color.GRAY.fill() else Color.WHITE.fill(),
                    Style.FILLED,
