 ...
 */
public class BPMNWithTimeAndDataDescription extends BPMNWithTimeGenerationDescription implements DescriptionWithDataObjects
{
    ...
    private Map<SubProcess, Collection<LoggableStringDataObject>> subProcessesToTheirInternalDataFlows =
            new HashMap<>();
    ...
    public BPMNWithTimeAndDataDescription(BPMNDiagram diagram)
    {
        super(diagram);
    ...
    private Map<DataObject, LoggableStringDataObject> initLoggableDataObjects(BPMNDiagram diagram)
    {
        ...
        for (DataObject dataObject : diagram.getDataObjects())
        {
            ...
            SubProcess dataObjectSubProcess = dataObject.getParentSubProcess();
            if (dataObjectSubProcess != null)
            {
                Collection<LoggableStringDataObject> internalDataObjects = subProcessesToTheirInternalDataFlows.get(dataObjectSubProcess);

                if (internalDataObjects == null)
                {
                    internalDataObjects = new ArrayList<>();
                    subProcessesToTheirInternalDataFlows.put(dataObjectSubProcess, internalDataObjects);
                }
            }
    ...
    public String getGatewayScriptPath(Gateway gateway)
    {
        return gatewaysToScriptPaths.get(gateway);
    }
